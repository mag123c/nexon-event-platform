...
describe('AuthController (E2E)', () => {
  ...
  const VALID_INTERNAL_API_KEY =
    process.env.GATEWAY_INTERNAL_API_KEY || 'TEST!';
  ...
  describe('POST /auth/register', () => {
    ...
    it('api key가 누락된 경우 401 Unauthorized를 반환해야 한다', async () => {
      ...
      expect(response.body.message).toBe(
        new MissingInternalApiKeyException().message,
      );
    });

    it('api key가 잘못된 경우 401 Unauthorized를 반환해야 한다', async () => {
      const response = await request(app.getHttpServer())
        .post('/auth/register')
        .set(CustomHeaders.INTERNAL_API_KEY, 'INVALID_API_KEY')
        .send(registerDto);

      expect(response.status).toBe(HttpStatus.UNAUTHORIZED);
      expect(response.body.message).toBe(
        new InvalidInternalApiKeyException().message,
      );
    });
    ...
    it('유효한 데이터로 요청 시 201 상태 코드와 사용자 정보를 반환해야 한다', async () => {
      const response = await request(app.getHttpServer())
        .post('/auth/register')
        .set(CustomHeaders.INTERNAL_API_KEY, VALID_INTERNAL_API_KEY)
        .send(registerDto);
