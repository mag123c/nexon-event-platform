...
import { Types, Connection, ClientSession } from 'mongoose'; // Connection, ClientSession 추가
...
} from '@app/event/domain/event-claim/ports/event-claim.repository';
import { EVENT_CLAIM_FACTORY } from '@app/event/domain/event-claim/factories/event-claim.factory';
import {
  USER_ACTIVITY_FETCHER,
  UserActivityFetcher,
} from '@app/event/application/ports/user-activity.fetcher';
...
import { EventClaimMongoFactory } from '@app/event/infrastructure/factories/event-claim-mongo.factory'; // 실제 Factory
import { EventConditionMatcherService } from '@app/event/application/services/event-condition-matcher.service'; // 실제 Service
import {
...
} from '@app/event/domain/event-claim/errors/event-claim.exception';
import { DatabaseOperationException } from '@app/common/errors/database-operation.exception';
import { ExternalServiceCommsException } from '@app/common/errors/external-service.exception'; // 경로 확인
...
describe('ClaimRewardUseCase', () => {
  ...
  describe('recordFailedClaim 헬퍼 메소드 동작', () => {
    it('실패 기록 중 DB 오류 발생 시에도 주 흐름에 영향을 주지 않아야 한다 (예외 전파 X)', async () => {
      ...
      // EventNotClaimableException이 발생하는지 확인 (recordFailedClaim의 에러는 무시되어야 함)
      await expect(useCase.execute(getBaseInput())).rejects.toThrow(
        EventNotClaimableException,
      );
    });
